<div class="container">
	<?php
	$query = mysqli_query($db, "SELECT*FROM lsp_buku WHERE no='$_GET[no]'");
...
<h3>Ubah Buku: <b><?php echo $data['judul']?></b></h3><hr>
	...
	<?php
	if (isset($_POST['ubah'])) {
		$ubah = mysqli_query($db, "UPDATE lsp_buku SET no='$_POST[no]', tanggal='$tgl', judul='$_POST[judul]', pengarang='$_POST[pengarang]', penerbit='$_POST[penerbit]', tahun_terbit='$_POST[tahun_terbit]', asal='$_POST[asal]', klasifikasi='$_POST[klasifikasi]' WHERE no='$_GET[no]'");
		$cekminjem = mysqli_num_rows(mysqli_query($db, "SELECT*FROM lsp_peminjaman WHERE no_buku='$_GET[no]'"));
		if ($cekminjem > 0) {
			mysqli_query($db, "UPDATE lsp_peminjaman SET no_buku='$_POST[no]' WHERE no_buku='$_GET[no]'");
		}
		if ($ubah) {
	...
	<form method="post">
		<table width="100%">
			...
			<tr>
				<td></td>
				<td></td>
				<td><button type="submit" name="ubah" class="btn btn-primary" style="width:25%">Ubah</button></td>
			</tr>
		</table>
	</form>
	</center>
